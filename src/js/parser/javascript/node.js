...
export default class Node {
  ...
  renderLabel(text) {
    let group = this.container.group().addClass('label'),
      rect = group.rect(),
    ...
    return this.deferredStep().then(() => {
      ...
      label.transform(
        Snap.matrix().translate(margin, box.height / 2 + 2 * margin)
      )
  ...
  renderLabeledBox(text, content, options) {
    let label = this.container
        .text(0, 0, _.flatten([text]))
    ...
    return this.deferredStep().then(() => {
      let labelBox = label.getBBox(),
        ...
        boxWidth = Math.max(
          contentBox.width + options.padding * 2,
          labelBox.width
        ),
      ...
      content.transform(
        Snap.matrix().translate(
          boxWidth / 2 - contentBox.cx,
          labelBox.height + options.padding
        )
      )
    })
  }
