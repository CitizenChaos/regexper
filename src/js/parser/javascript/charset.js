...
export default {
  ...
  _render() {
    ...
    return Promise.all(
      _.map(this.elements, (part) => part.render(this.partContainer.group()))
    ).then(() => {
  ...
  setup() {
    ...
    this.label =
      this.properties.invert.textValue === '^' ? 'None of:' : 'One of:'
    ...
    this.elements = _.uniqBy(
      this.properties.parts.elements,
    ...
    if (this.textValue.match(/\\c[^a-zA-Z]/)) {
      this.state.warnings.push(
        `The character set "${this.textValue}" contains the \\c escape followed by a character other than A-Z. This can lead to different behavior depending on browser. The representation here is the most common interpretation.`
      )
    }
  }
}
