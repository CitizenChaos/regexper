...
export default {
  ...
  _render() {
    let matchContainer = this.container
      .group()
    ...
    return Promise.all(
      _.map(this.matches, (match) => match.render(matchContainer.group()))
    ).then(() => {
      let containerBox, paths
      ...
      paths = _.map(this.matches, (match) =>
        this.makeCurve(containerBox, match)
      )
  ...
  setup() {
    if (this.properties.alternates.elements.length === 0) {
      ...
      this.matches = [this.properties.match].concat(
        _.map(
          this.properties.alternates.elements,
