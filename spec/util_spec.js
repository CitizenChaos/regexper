...
describe('util.js', function () {
  ...
  describe('spaceHorizontally', function () {
    it('positions each item', function () {
      var svg = Snap(document.createElement('svg')),
        items = [svg.group(), svg.group(), svg.group()]
      ...
      expect(items[0].transform).toHaveBeenCalledWith(
        Snap.matrix().translate(0, 10)
      )

      expect(items[1].transform).toHaveBeenCalledWith(
        Snap.matrix().translate(15, 0)
      )

      expect(items[2].transform).toHaveBeenCalledWith(
        Snap.matrix().translate(50, 5)
      )
    })
  })
  ...
  describe('spaceVertically', function () {
    it('positions each item', function () {
      ...
      expect(items[0].transform).toHaveBeenCalledWith(
        Snap.matrix().translate(10, 0)
      )

      expect(items[1].transform).toHaveBeenCalledWith(
        Snap.matrix().translate(0, 15)
      )

      expect(items[2].transform).toHaveBeenCalledWith(
        Snap.matrix().translate(5, 50)
      )
    })
  })
})
