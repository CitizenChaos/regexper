...
describe('parser/javascript/regexp.js', function () {
  _.forIn(
    ...
    (content, str) => {
      it(`parses "${str}" as a Regexp`, function () {
        ...
        expect(parser.__consume__regexp()).toEqual(
  ...
  describe('#_render', function () {
    ...
    it('creates a container for the match nodes', function () {
      ...
      expect(this.group.transform).toHaveBeenCalledWith(
        Snap.matrix().translate(20, 0)
      )
    })
    ...
    describe('positioning of the match nodes', function () {
      ...
      it('renders the sides and curves into the container', function (done) {
        this.node
          ._render()
          .then(() => {
            expect(this.node.makeCurve).toHaveBeenCalledWith(
              'container bbox',
              this.node.matches[0]
            )
            expect(this.node.makeCurve).toHaveBeenCalledWith(
            ...
            expect(this.node.makeSide).toHaveBeenCalledWith(
              'container bbox',
              this.node.matches[0]
            )
            expect(this.node.makeSide).toHaveBeenCalledWith(
            ...
            expect(this.node.container.path).toHaveBeenCalledWith(
              'curvecurvecurvesideside'
            )
            done()
          })
          .catch(done.fail)
      ...
      it('renders the connectors into the match container', function (done) {
        this.node
          ._render()
          .then(() => {
            expect(this.node.makeConnector).toHaveBeenCalledWith(
              'group bbox',
              this.node.matches[0]
            )
            expect(this.node.makeConnector).toHaveBeenCalledWith(
            ...
            expect(this.group.path).toHaveBeenCalledWith(
  ...
  describe('#madeSide', function () {
    ...
    describe('when the match node is less than 15px from the centerline', function () {
      ...
      it('returns nothing', function () {
        expect(
          this.node.makeSide(this.containerBox, this.match)
        ).toBeUndefined()
      })
    })
  })
  ...
  describe('#makeConnector', function () {
    ...
    it('returns a line from the curve to the match node', function () {
      expect(this.node.makeConnector(this.containerBox, this.match)).toEqual(
        'M0,1h2M3,1H4'
      )
    })
  })
