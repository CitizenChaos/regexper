...
describe('parser/javascript/charset.js', function () {
  _.forIn(
    ...
    (content, str) => {
      it(`parses "${str}" as a Charset`, function () {
        ...
        expect(parser.__consume__charset()).toEqual(
  ...
  it('adds a warning for character sets the contain non-standard escapes', function () {
    ...
    expect(node.state.warnings).toEqual([
      'The character set "[\\c]" contains the \\c escape followed by a character other than A-Z. This can lead to different behavior depending on browser. The representation here is the most common interpretation.'
    ])
  })
  ...
  describe('#_render', function () {
    beforeEach(function () {
      ...
      this.node.elements[0].render.and.returnValue(
        this.elementDeferred[0].promise
      )
      this.node.elements[1].render.and.returnValue(
        this.elementDeferred[1].promise
      )
      this.node.elements[2].render.and.returnValue(
        this.elementDeferred[2].promise
      )
      ...
      spyOn(this.node, 'renderLabeledBox').and.returnValue(
        'labeled box promise'
      )
    ...
    describe('positioning of the items', function () {
      ...
      it('spaces the elements vertically', function (done) {
        this.node
          ._render()
          .then(() => {
            expect(util.spaceVertically).toHaveBeenCalledWith(
              this.node.elements,
              { padding: 5 }
            )
            done()
      ...
      it('renders a labeled box', function (done) {
        this.node
          ._render()
          .then((result) => {
            expect(this.node.renderLabeledBox).toHaveBeenCalledWith(
              'example label',
              this.partContainer,
              {
                padding: 5
