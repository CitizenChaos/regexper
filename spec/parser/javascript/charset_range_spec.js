...
describe('parser/javascript/charset_range.js', function () {
  _.forIn(
    ...
    (content, str) => {
      it(`parses "${str}" as a CharsetRange`, function () {
        ...
        expect(parser.__consume__charset_range()).toEqual(
  ...
  _.each(['\\d-a', '\\D-a', '\\s-a', '\\S-a', '\\w-a', '\\W-a'], (str) => {
  ...
  describe('#_render', function () {
    beforeEach(function () {
      ...
      this.node.container = jasmine.createSpyObj('cotnainer', [
        'addClass',
        'text',
        'group'
      ])
      ...
      spyOn(this.node.first, 'render').and.returnValue(
        this.firstDeferred.promise
      )
      ...
      this.node
        ...
        .then(() => {
          expect(util.spaceHorizontally).toHaveBeenCalledWith(
            [this.node.first, 'hyphen', this.node.last],
            { padding: 5 }
          )
          done()
        })
