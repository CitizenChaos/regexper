  ...
  describe('#renderLabel', function () {
    ...
    describe('positioning of label elements', function () {
      ...
      it('transforms the text element', function (done) {
        this.node
          .renderLabel('example label')
          .then(() => {
            expect(this.text.transform).toHaveBeenCalledWith(
              Snap.matrix().translate(5, 22)
            )
            done()
          })
          .catch(done.fail)
  ...
  describe('#renderLabeledBox', function () {
    ...
    describe('positioning of elements', function () {
      ...
      it('positions the text element', function (done) {
        ...
        this.node
          .renderLabeledBox('example label', this.content, { padding: 5 })
          .then(() => {
            expect(this.text.transform).toHaveBeenCalledWith(
              Snap.matrix().translate(0, 20)
            )
            done()
          })
          .catch(done.fail)
      ...
      it('positions the rect element', function (done) {
        ...
        this.node
          .renderLabeledBox('example label', this.content, { padding: 5 })
          .then(() => {
            expect(this.rect.transform).toHaveBeenCalledWith(
              Snap.matrix().translate(0, 20)
            )
            done()
          })
      ...
      it('positions the content element', function (done) {
        ...
        this.node
          .renderLabeledBox('example label', this.content, { padding: 5 })
          .then(() => {
            expect(this.content.transform).toHaveBeenCalledWith(
              Snap.matrix().translate(5, 25)
            )
            done()
          })
          .catch(done.fail)
