  ...
  _.forIn(
    ...
    (content, str) => {
      it(`parses "${str}" as a MatchFragment`, function () {
        ...
        expect(parser.__consume__match_fragment()).toEqual(
    ...
    describe('positioning of content', function () {
      ...
      it('moves the content to the correct position', function (done) {
        this.node
          ._render()
          .then(() => {
            expect(this.node.content.transform).toHaveBeenCalledWith(
              'example position'
            )
            done()
          })
      ...
      it('renders a skip path and loop path', function (done) {
        this.node
          ._render()
          .then(() => {
            expect(this.node.repeat.skipPath).toHaveBeenCalledWith(
              'content bbox'
            )
            expect(this.node.repeat.loopPath).toHaveBeenCalledWith(
              'content bbox'
            )
    ...
    describe('when a label is defined', function () {
      ...
      describe('when there is a skip loop', function () {
        ...
        it('positions the text element', function () {
          ...
          expect(this.text.transform).toHaveBeenCalledWith(
            Snap.matrix().translate(17, 66)
          )
        })
      })
