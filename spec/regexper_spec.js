...
describe('regexper.js', function () {
  ...
  describe('#submitListener', function () {
    ...
    describe('when setting location.hash fails', function () {
      ...
      it('shows the expression directly', function () {
        ...
        expect(this.regexper.showExpression).toHaveBeenCalledWith(
          'example value'
        )
      })
    })
  ...
  describe('#hashchangeListener', function () {
    describe('when the URL is invalid', function () {
      ...
      it('displays an error message', function () {
        ...
        expect(this.regexper.error.innerHTML).toEqual(
          'Malformed expression in URL'
        )
      })
    })
  ...
  describe('#bindListeners', function () {
    ...
    it('binds #keypressListener to keypress on the text field', function () {
      ...
      expect(this.regexper.field.addEventListener).toHaveBeenCalledWith(
        'keypress',
    ...
    it('binds #submitListener to submit on the form', function () {
      ...
      expect(this.regexper.form.addEventListener).toHaveBeenCalledWith(
        'submit',
        jasmine.any(Function)
      )
    ...
    it('binds #documentKeypressListener to keyup on the root', function () {
      ...
      expect(this.regexper.root.addEventListener).toHaveBeenCalledWith(
        'keyup',
        jasmine.any(Function)
      )
  ...
  describe('#showExpression', function () {
    beforeEach(function () {
      spyOn(this.regexper, 'renderRegexp').and.returnValue(
        jasmine.createSpyObj('renderRegexp', ['catch'])
      )
    })
    ...
    describe('when the expression is not blank', function () {
      it('renders the expression', function () {
        ...
        expect(this.regexper.renderRegexp).toHaveBeenCalledWith(
          'example expression'
        )
      })
    })
  ...
  describe('#updateLinks', function () {
    beforeEach(function () {
      ...
      this.regexper.svgContainer.innerHTML =
        '<div class="svg">example image</div>'
    })
    ...
    describe('when blob URLs are supported', function () {
      ...
      it('sets the download link href', function () {
        ...
        expect(this.regexper.downloadSvg.href).toEqual(
  ...
  describe('#displayWarnings', function () {
    it('adds a list item for each warning', function () {
      ...
      expect(this.regexper.warnings.innerHTML).toEqual(
        '<li class="inline-icon">(icon-markup)warning 1</li><li class="inline-icon">(icon-markup)warning 2</li>'
      )
    })
  })
  ...
  describe('#renderRegexp', function () {
    beforeEach(function () {
      ...
      spyOn(Parser.prototype, 'parse').and.returnValue(
        this.parsePromise.promise
      )
      spyOn(Parser.prototype, 'render').and.returnValue(
    ...
    it('parses the expression', function () {
      ...
      expect(this.regexper.running.parse).toHaveBeenCalledWith(
        'example expression'
      )
    })
    ...
    describe('when parsing fails', function () {
      ...
      it('sets the state to be "has-error"', function (done) {
        this.regexper
          .renderRegexp('example expression')
          .then(() => {
            expect(this.regexper.state).toEqual('has-error')
            done()
          })
